...
#### How it Works (RFC 8439 Section 2.8)

1. Generate Poly1305 key using ChaCha20 block function with counter=0
2. Encrypt plaintext using ChaCha20 with counter starting at 1
3. Construct MAC data: `AAD || pad16(AAD) || ciphertext || pad16(ciphertext) || len(AAD) || len(ciphertext)`
4. Compute Poly1305 MAC over the constructed data
5. Return ciphertext and tag
...
The ChaCha20 block function operates on a 4x4 matrix of 32-bit words:
...
- `k` = 256-bit key
- `b` = block counter (32-bit)
- `n` = 96-bit nonce

The function performs 20 rounds (10 double-rounds) of quarter-round operations, then adds the original state to produce 64 bytes of keystream.

### Poly1305 Key Generation

The Poly1305 one-time key is generated by:
1. Running ChaCha20 block function with counter=0
2. Taking the first 32 bytes as the Poly1305 key

### Padding Function

For AEAD construction, data is padded to 16-byte boundaries:
```python
pad16(x) = if len(x) % 16 == 0: [] else: [0] * (16 - len(x) % 16)
```

## Files

- `chacha20.py` - ChaCha20 stream cipher implementation
- `aead_wrap.py` - AEAD encryption and authentication
- `aead_unwrap.py` - AEAD verification and decryption
- `keyfile` - Test key file (binary, 32 bytes)
- `sunscreen.txt` - Test plaintext from RFC 8439
- `ciphertext.bin` - Expected ciphertext for sunscreen test
...
- Section 2.1: The Quarter Round
- Section 2.2: A Quarter Round on the ChaCha State
- Section 2.3: The ChaCha20 Block Function
- Section 2.4: The ChaCha20 Encryption Algorithm
- Section 2.6: Generating the Poly1305 Key Using ChaCha20
